...
```{r 模擬參數路徑, echo=FALSE}
X=m.data
X[,"預期價格漲幅百分比"]=X[,"預期價格漲幅百分比"]*100
ggplot(data =X, aes(x=估計參數, y=預期價格漲幅百分比)) + geom_point()+geom_hline(yintercept =10,linetype='dotted')+ geom_line(aes(colour=參與結合廠商))+ylab("結合後預期價格上漲(%)")+xlab("估計參數")+theme_classic()
datatable(format(m.data),options = list(pageLength =nrow(X)))%>%formatPercentage(c("預期價格漲幅百分比"), 1) 
```
